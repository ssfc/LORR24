...
std::pair<double, int> PIBT::search(uint32_t r, int banned_direction) {
    ...
    const double mult = (robots.size() - robots[r].priority) * 1.0 / robots.size();

    for(uint32_t k = 0; k < actions.size(); k++){
        auto [_, dir] = actions[k];
        ...
        if (!pos_to_robot.count(to.pos)) {
            ...
            auto [score, _] = search(to_r, (dir + 2) % 4);
            if (score >= 0) {
                score += mult * (k + 1);
                if (score < best_score) {
                    best_score = score;
...
bool PIBT::build(uint32_t r, int banned_direction) {
    auto [score, dir] = search(r, banned_direction);
    if (score < 0) {
        return false;
    }
