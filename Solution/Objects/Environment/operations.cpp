...
// 实现了对某个“操作序列”是否合法的验证逻辑
// comment: 检查出一些浪费步数的操作
bool verify_operation(const Operation &op) {
    ...
    // операции вида CWC плохие, потому что мы повернулись, подождали и повернулись еще раз 像CWC这种操作是不好的，因为我们转了一次，等待了一下，又再转了一次。
    // comment: 确实, 直接转就好了
    FAILED_ASSERT("TODO");
