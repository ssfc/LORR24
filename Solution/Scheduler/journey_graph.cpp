...
int get_dist123(const uint32_t& from, const uint32_t to) {
...
Path::Path(PathSegment* start_segment, JourneyGraph& jg): start(start_segment), end(jg.findLastSegmentInPath(start_segment)) {
    ...
    jg.nodes[jg.task_to_nodes[jg.edges[start_segment->edge_id].task_id].second].edge_taken = start_segment->edge_id;
}
