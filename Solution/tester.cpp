...
uint32_t call(const std::string &test, int steps_num, uint32_t test_id) {
    ...
    int ret_code = std::system(
            ("./cmake-build-release-remote-host/lifelong -i " + test + " -o Tmp/test" + std::to_string(test_id) + ".json -s " + std::to_string(steps_num) + " -t 100000 -p 1000000000 --unique_id " + std::to_string(test_id) + " > Tmp/log" + std::to_string(test_id) + ".txt").c_str());
