...
uint32_t call(const std::string &test, uint32_t test_id) {
    ...
    int ret_code = std::system(
            ("./cmake-build-release-wsl/lifelong -i example_problems/" + test + " -o Tmp/test" + std::to_string(test_id) + ".json -s 1000 -t 10000 -p 100000000 --unique_id " + std::to_string(test_id) + " > Tmp/log" + std::to_string(test_id) + ".txt").c_str());
